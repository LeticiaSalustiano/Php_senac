          ...
          if(isset($_GET['usuario']) && $_GET['usuario'] == 'adm'){?>
            ...
            if($qtd > 0){
                ...
                while($row = $res->fetch_object()){
                    ...
                    print "<td><button class='btn btn-success btn-sm' onclick=\"location.href='autorizacao.php?id=". $row -> id_usuario . "&autorizar=simA';\">Sim</button>
                    <button class='btn btn-danger btn-sm' onclick=\"location.href='autorizacao.php?id=". $row -> id_usuario . "&autorizar=nao';\">Não</button></td>"; } 
                    else if ($row -> perfil == 'Tecnico'){
                      print "<td><button class='btn btn-success btn-sm' onclick=\"location.href='autorizacao.php?id=". $row -> id_usuario . "&autorizar=simT';\">Sim</button>
                    <button class='btn btn-danger btn-sm' onclick=\"location.href='autorizacao.php?id=". $row -> id_usuario . "&autorizar=nao';\">Não</button></td>";}
